...
function Inicio({navigation}) {
    ...
    useEffect(() => {
        const obtenerDatosNegocios = async () => {
            try {
                const response = await fetch("http://192.168.1.68:3000/inicionegocio");
                if (!response.ok) {
    ...
    useEffect(() => {
        const obtenerDatosProductos = async () => {
            try {
                const response = await fetch("http://192.168.1.68:3000/inicioproducto");
                if (!response.ok) {
                    throw new Error('Error en la solicitud: ' + response.status);
                }
    ...
    useEffect(() => {
        const obtenerDatosNovedades = async () => {
            try {
                const response = await fetch("http://192.168.1.68:3000/inicionovedad");
                if (!response.ok) {
                    throw new Error('Error en la solicitud: ' + response.status);
                }
